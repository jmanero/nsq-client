...
function framer(data) {
  ...
  switch (type) {
    case 0: // Response/Control
      ...
      // Close transactions
      if (typeof protocol.commit === "function") protocol.commit(null, message);
      break;
    case 1: // Error
      ...
      // Close transactions
      if (typeof protocol.commit === "function") protocol.commit(err);
      break;
    case 2: // Message
      try {
  ...
  if (data.length - 4 - size > 0) framer.call(protocol, data.slice(size + 4));
}
...
Protocol.prototype.subscribe = function(topic, channel, callback) {
  // Validation
  if (!Policy.validTopic(topic)) throw NSQProtocolError("E_BAD_TOPIC");
  if (!Policy.validChannel(channel)) throw NSQProtocolError("E_BAD_CHANNEL");
